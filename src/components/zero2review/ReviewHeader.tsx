...
export default function ReviewHeader({ stage, dailyMinutes = 30, onStart }: { stage?: Zero2ReviewStage; dailyMinutes?: number; onStart?: () => void }) {
  return <header className="page-hero review-hero flex flex-wrap items-center justify-between gap-4">
    <div className="page-hero-copy"><div className="page-kicker flex items-center gap-1"><Brain className="h-3.5 w-3.5" /> zero2Agent / 面试通关</div><h1 className="text-xl font-bold">Agent 面试训练营</h1><p className="page-subtitle">围绕课程原文建立可验证的回答，再用诊断题确认真正掌握。</p></div>
