...
export default function DiagnosticQuestion({ question, answer, evaluation, onAnswer, onSubmit, onCorrect }: { question?: Zero2ReviewQuestion; answer: string; evaluation?: Zero2EvaluationDraft; onAnswer: (value: string) => void; onSubmit: () => void; onCorrect?: (score: 0 | 1 | 2 | 3 | 4) => void }) {
  ...
  if (!question) {
    ...
    return <section className="card space-y-2 border-[var(--color-success)]/30 p-4"><h2 className="font-semibold">诊断完成 · 本次评价 {evaluation.score}/4</h2><p className="text-xs text-[var(--color-text-secondary)]">掌握度已经更新；接下来可以复习薄弱点，再开始下一题。</p>{evaluation.missingPoints.length > 0 && <p className="text-sm text-[var(--color-text-secondary)]">遗漏：{evaluation.missingPoints.join('；')}</p>}{onCorrect && <div className="flex flex-wrap gap-1 text-xs"><span className="self-center text-[var(--color-text-tertiary)]">修正评分：</span>{([0, 1, 2, 3, 4] as const).map((score) => <button key={score} className={`btn-ghost ${evaluation.score === score ? 'border-[var(--color-primary)]' : ''}`} onClick={() => onCorrect(score)}>{score}</button>)}</div>}</section>;
  }
