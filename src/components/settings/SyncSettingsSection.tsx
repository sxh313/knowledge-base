    ...
    <section id="cloud-sync" className="scroll-mt-6 space-y-3">
      <details className="settings-disclosure card group" open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
        ...
        {config.enabled && <>
          ...
          <label className="flex items-center gap-2 cursor-pointer" title="默认只同步 zero2Agent 掌握度、计划和任务；开启后才同步复习问答与作答正文"><input type="checkbox" checked={config.syncZero2ReviewHistory ?? false} onChange={e => onUpdate({ syncZero2ReviewHistory: e.target.checked })} className="h-4 w-4 rounded border-[var(--color-border)]" /><span className="text-sm">同步 zero2Agent 问答历史（敏感内容，默认关闭）</span></label>
