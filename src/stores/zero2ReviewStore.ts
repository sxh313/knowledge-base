...
import { handleZero2Input, restoreZero2Session, submitZero2Answer, finishTask, skipTask, deleteTask, correctAttemptScore, createZero2ReviewOrchestrator, type OrchestratorState } from '../lib/zero2review/orchestrator';
...
export const useZero2ReviewStore = create<Zero2ReviewStore>((set, get) => ({
  ...
  deleteTask: async (taskId) => { await deleteTask(taskId); set((store) => ({ tasks: store.tasks.filter((task) => task.id !== taskId) })); },
